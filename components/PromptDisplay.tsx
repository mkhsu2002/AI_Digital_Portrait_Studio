    ...
    if (isLoading) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-slate-400">
          ...
          <p className="mt-4 text-lg">正在產生三種不同視角的圖片...</p>
          <p className="text-sm">這可能需要一點時間，請稍候。</p>
        </div>
      );
    }
      ...
      return (
        <div className="flex items-center justify-center h-full text-red-400">
          <div className="text-center">
            <h3 className="text-lg font-semibold">產生失敗</h3>
      ...
      return (
        <div className="grid grid-cols-1 gap-6">
          {images.map((image, index) => {
            ...
            const filename = `${sanitizedProductName}_${image.label}_${Date.now()}.${fileExtension}`;

            return (
              <div key={index} className="bg-slate-900/50 rounded-lg overflow-hidden border border-slate-700 flex flex-col">
                <div className="aspect-[3/4] bg-slate-900 flex items-center justify-center">
                  {image.videoSrc ? (
                    <video src={image.videoSrc} controls autoPlay loop muted className="w-full h-full object-cover" />
                  ) : (
    ...
    return (
       <div className="flex items-center justify-center h-full">
        <div className="text-center text-slate-500">
          ...
          <h3 className="mt-2 text-sm font-medium">圖片將會顯示在這裡</h3>
          <p className="mt-1 text-sm">點擊「產生圖片」按鈕開始。</p>
        </div>
      </div>
    );
  };
    ...
    <div>
      <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700 h-full flex flex-col">
        <div className="flex justify-between items-start mb-4">
           <div>
            <h2 className="text-2xl font-semibold text-slate-200">產生結果</h2>
            <p className="text-sm text-slate-400 mt-1">AI 產生的三種不同視角圖片</p>
          </div>
          <button
            onClick={handleCopy}
        ...
        <details className="mt-4 text-sm">
          <summary className="cursor-pointer text-slate-400 hover:text-slate-200">顯示/隱藏詠唱內容</summary>
